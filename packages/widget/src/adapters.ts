...
export function createComponentFactory<TProps extends WidgetProps>() {
  return {
     ...
     * 创建高阶基类组件
     */
    advanced: <T extends Widget<TProps>>(
      WidgetClass: new (props: TProps) => T,
    ) => WidgetClass,

    /**
     * 创建标准组件函数
     */
    component: (renderFn: (props: TProps) => VNode) =>
      renderFn as ComponentFunction,
  };
}
...
export const is = {
  ...
  simpleWidget: (
    obj: any,
  ): obj is ReturnType<ReturnType<typeof createWidget>> => {
    return (
      obj &&
