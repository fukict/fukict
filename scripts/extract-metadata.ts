...
async function main(): Promise<void> {
  log('blue', '📦 Extracting metadata from packages...\n');
  ...
  for (const pkg of packages) {
    ...
    if (!config || config.metadata !== true) {
      log('yellow', `⏭️  Skipped ${pkg}: metadata generation disabled`);
  ...
  if (failedPackages.length > 0) {
    log('red', `❌ Failed to extract metadata: ${failedPackages.join(', ')}`);
    process.exit(1);
  } else {
    ...
    if (generatedCount > 0) {
      log(
        'green',
        `✅ Successfully extracted metadata for ${generatedCount} package(s)`,
      );
    }
    if (skippedPackages.length > 0) {
      log(
        'cyan',
        `ℹ️  Skipped ${skippedPackages.length} package(s): ${skippedPackages.join(', ')}`,
      );
    }
  }
}
